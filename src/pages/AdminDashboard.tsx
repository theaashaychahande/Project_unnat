...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "queue" | "map">(
    "overview",
  );
  ...
  const getUrgencyBadge = (urgency: Urgency) => {
    switch (urgency) {
      ...
      case "High":
        return "border-orange text-orange bg-orange/10";
      case "Critical":
      ...
      <aside className="w-72 bg-gov-white border-r border-gov-border flex flex-col h-screen sticky top-0 z-50 shadow-sm">
        <div className="p-8 border-b border-gov-border">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gov-green-primary rounded-lg flex items-center justify-center font-bold text-gov-white text-lg">
              U
            </div>
            <h1 className="text-2xl font-baskerville font-bold text-gov-green-dark">
              UNNAT
            </h1>
          </div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary mt-4">
            Admin Control
          </p>
        </div>
        ...
        <nav className="flex-1 p-6 space-y-2">
          ...
          >
            <LayoutDashboard
              ...
              className={
                activeTab === "overview"
                  ? "text-gov-green-dark"
                  : "group-hover:text-gov-green-primary"
              }
            />
          ...
          <div className="pt-8 opacity-20 border-t border-gov-border mt-8">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary mb-4 px-6">
              System Management
            </p>
        ...
        <div className="p-8 border-t border-gov-border bg-gov-background/50">
          <div className="flex items-center space-x-4 mb-6">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-gov-green-primary font-bold uppercase tracking-widest">
                Master Admin
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 overflow-y-auto h-screen relative">
        <header className="sticky top-0 z-40 bg-gov-white/80 backdrop-blur-xl border-b border-gov-border px-10 py-6 flex justify-between items-center">
          <div className="relative max-w-md w-full">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-gov-text-secondary"
              size={18}
            />
            <input
              type="text"
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="text-right">
              <p className="text-[10px] font-bold text-gov-text-secondary uppercase tracking-widest">
                Current Status
              </p>
        ...
        <div className="p-10 max-w-7xl mx-auto">
          <div className="mb-12">
            <h2 className="text-4xl font-black font-baskerville tracking-tight capitalize mb-2">
              {activeTab}
            </h2>
            <p className="text-gov-text-secondary font-bold uppercase tracking-[0.2em] text-xs">
              ...
              <div key="overview" className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {[
                    ...
                    <div
                      key={i}
                      className="card-gov p-8 border-l-4 border-l-gov-green-primary relative overflow-hidden group"
                    >
                      <div className="absolute -right-4 -bottom-4 opacity-[0.03] group-hover:scale-110 transition-transform">
                        <s.icon size={120} />
                      </div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-gov-text-secondary mb-2">
                        {s.label}
                      </p>
                      <h3 className="text-5xl font-black font-syne text-gov-green-dark tracking-tighter">
                        {s.val}
                      </h3>
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 card-gov p-8">
                    <h3 className="text-xl font-black font-syne tracking-tight mb-8 text-gov-text-primary">
                        ...
                        ([level, count]) => (
                          <div key={level}>
                            <div className="flex justify-between items-end mb-2">
                              <span className="text-[10px] font-black uppercase tracking-widest text-gov-text-secondary">
                                {level} Urgency
                              </span>
                              <span className="text-sm font-black text-gov-text-primary">
                                {count}
                              </span>
                            </div>
                  ...
                  <div className="card-gov p-8 bg-gov-green-light/20 border-gov-green-primary/10 relative overflow-hidden">
                    ...
                    <h3 className="text-xl font-baskerville font-bold tracking-tight mb-6 text-gov-green-dark">
                      Critical Briefing
                    </h3>
                    <div className="space-y-4">
                      {complaints
                        .filter(
                          (c) =>
                            c.urgency === "Critical" && c.status !== "Resolved",
                        )
                        .slice(0, 3)
                        .map((c) => (
                          <div
                            key={c.id}
                            className="p-4 bg-gov-white border border-gov-border rounded-xl hover:border-gov-green-primary transition-all cursor-pointer"
                          >
                            <p className="text-xs font-bold text-gov-text-primary truncate">
                              {c.title}
                            </p>
                            <p className="text-[10px] text-gov-text-secondary font-bold uppercase mt-1">
                              📍 {c.location}
                            </p>
                          </div>
                        ))}
                      {stats.urgencyBreakdown.Critical === 0 && (
                        <div className="py-8 text-center">
                          <CheckCircle2
                            size={32}
                            className="mx-auto text-gov-green-primary opacity-20 mb-3"
                          />
                          <p className="text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary">
                            No Critical Alerts
                          </p>
                        </div>
                      )}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-black font-syne tracking-tight text-gov-green-dark">
                      Geographical Distribution
                    </h3>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary">
                      Nagpur City Map
                    </span>
                  </div>
                  ...
                  <div className="flex gap-4">
                    {(["urgency", "status"] as const).map((fType) => (
                      <div key={fType} className="relative group">
                        ...
                        <ChevronDown
                          size={14}
                          className="absolute right-4 top-1/2 -translate-y-1/2 text-gov-text-secondary pointer-events-none"
                        />
                      </div>
                    ))}
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-gov-background/50 border-b border-gov-border">
                        <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary">
                          Citizen Profile
                        </th>
                        <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary">
                          Docket Details
                        ...
                        <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-gov-text-secondary text-right">
                          Update Action
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gov-border">
                      {filteredComplaints.map((c) => (
                        <tr
                          key={c.id}
                          className="hover:bg-gov-background/30 transition-all group"
                        >
                          <td className="px-8 py-6">
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <p className="text-sm font-bold text-gov-text-primary">
                                  {c.userName}
                                </p>
                                <p className="text-[10px] text-gov-text-secondary font-bold uppercase tracking-tighter italic">
                          ...
                          <td className="px-8 py-6">
                            <span
                              className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase border-2 ${getUrgencyBadge(c.urgency)}`}
                            >
                              {c.urgency}
                            </span>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex items-center space-x-3">
                              <div
                                className={`w-2 h-2 rounded-full ${c.status === "Resolved" ? "bg-gov-green-primary shadow-[0_0_8px_rgba(45,106,79,0.4)]" : c.status === "In Progress" ? "bg-blue-500" : "bg-yellow-500"}`}
                              ></div>
                          ...
                          <td className="px-8 py-6 text-right">
                            <div className="inline-flex items-center relative group/select">
                              ...
                              <ChevronDown
                                size={12}
                                className="absolute right-3 top-1/2 -translate-y-1/2 text-gov-text-secondary pointer-events-none"
                              />
                            </div>
                          </td>
                        </tr>
                  ...
                  {filteredComplaints.length === 0 && (
                    <div className="p-20 text-center">
                      ...
                      <p className="text-gov-text-secondary font-medium uppercase tracking-[0.2em] text-xs">

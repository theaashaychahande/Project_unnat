...
const CitizenDashboard: React.FC = () => {
  const { currentUser, logout, complaints, addComplaint, loading } =
    useAppContext();
  ...
  const getUrgencyStyles = (urgency: Urgency) => {
    switch (urgency) {
      case "Low":
        return "border-gov-green-light text-gov-green-dark bg-gov-green-light";
      case "Medium":
        return "border-yellow-400 text-yellow-600 bg-yellow-100";
      case "High":
        return "border-orange-400 text-orange-600 bg-orange-100";
      case "Critical":
        return "border-red-400 text-red-600 bg-red-100";
      default:
        return "border-gov-border text-gov-text-secondary bg-gov-background";
    }
  };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Pending":
        return <Clock size={14} className="animate-pulse text-yellow-500" />;
      case "In Progress":
        return (
          <AlertCircle size={14} className="animate-bounce text-blue-500" />
        );
      case "Resolved":
        return <CheckCircle2 size={14} className="text-gov-green-primary" />;
      default:
        return null;
    }
  };
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3 pl-4 border-l border-gov-border">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-bold uppercase tracking-widest text-gov-green-primary">
                  {currentUser.name}
                </p>
                <p className="text-[10px] text-gov-text-secondary font-bold uppercase">
                  Citizen
                </p>
              </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
          ...
          >
            <div>
              <h2 className="text-4xl font-baskerville font-bold text-gov-green-dark mb-2">
                Welcome,{" "}
                <span className="text-gov-green-primary">
              ...
              <div className="relative w-16 h-16">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle
                    ...
                    strokeWidth="4"
                    strokeDasharray={175}
                    strokeDashoffset={175 * (1 - 0.85)}
                    className="text-gov-green-primary"
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center font-bold text-sm text-gov-green-dark">
                  85
                </div>
              </div>
              <div>
                ...
                <p className="text-xs font-bold text-gov-green-primary">
                  Excellent Standing
                </p>
              </div>
            </div>
          ...
          {!showForm ? (
            ...
            >
              ...
              <div className="text-center">
                <h3 className="text-xl font-baskerville font-bold text-gov-green-dark">
                  File a Complaint
                </h3>
                <p className="text-sm text-gov-text-secondary font-medium">
                  Help improve Nagpur's civic services.
                </p>
              </div>
            </motion.button>
          ) : (
            ...
            >
              <div className="bg-gov-green-primary p-4 flex justify-between items-center">
                <h3 className="font-bold font-baskerville text-gov-white uppercase tracking-widest text-sm">
                  New Complaint Form
                </h3>
                <button
                  onClick={() => setShowForm(false)}
                  className="text-gov-white/70 hover:text-gov-white transition-colors font-bold text-xl"
                >
                  ✕
                </button>
              </div>
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-gov-green-primary ml-2">
                      Complaint Title
                    </label>
                    <input
                      name="title"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <MapPin
                        size={16}
                        className="absolute left-4 top-1/2 -translate-y-1/2 text-gov-green-primary"
                      />
                      <input
                        name="location"
                ...
                <div className="flex flex-col sm:flex-row gap-6 items-center">
                  <div className="w-full sm:flex-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-gov-green-primary ml-2 mb-2 block">
                      Visual Evidence
                    </label>
                  ...
                  <button
                    type="submit"
                    className="btn-gov-primary w-full sm:w-auto px-12 py-4 text-xs tracking-widest uppercase"
                  >
                    Submit Complaint
                  </button>
                </div>
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between">
            <h3 className="text-2xl font-black font-syne tracking-tight">
              Complaint History
            </h3>
          ...
          <div className="relative space-y-6 before:absolute before:left-[19px] before:top-4 before:bottom-4 before:w-px before:bg-gov-border">
            {userComplaints.length === 0 ? (
              <div className="card-gov p-12 text-center">
                <p className="text-gov-text-secondary font-medium">
                ...
                {userComplaints.map((complaint, idx) => (
                  ...
                  >
                    <div className="absolute left-0 top-6 w-10 h-10 rounded-full bg-gov-white border-4 border-gov-background z-10 flex items-center justify-center shadow-sm">
                      <div
                        className={`w-3 h-3 rounded-full ${complaint.urgency === "Critical" ? "bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]" : "bg-gov-green-primary"}`}
                      ></div>
                    </div>
                    ...
                    <div className="card-gov p-6 group-hover:border-gov-green-primary transition-all group-hover:translate-x-1">
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
                        <div>
                          <h4 className="text-xl font-baskerville font-bold text-gov-green-dark mb-1">
                            {complaint.title}
                          </h4>
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold uppercase border-2 ${getUrgencyStyles(complaint.urgency)}`}
                          >
                            {complaint.urgency}
                          </span>
                      ...
                      <p className="text-gov-text-secondary text-sm leading-relaxed line-clamp-2">
